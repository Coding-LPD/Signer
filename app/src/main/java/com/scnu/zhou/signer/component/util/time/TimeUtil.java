...
public class TimeUtil {
     ...
     * @throws
     */
    public static String setDaysForNow(long timeInMillis){
        //系统当前时间
        Calendar CD = Calendar.getInstance();
        long cTimeInMillis = CD.getTimeInMillis();

        //计算天数的间隔
        long duration = cTimeInMillis - timeInMillis;
        long Dduration = duration/(1000*3600*24);
        if (Dduration > 5){
            SimpleDateFormat dayformat= new SimpleDateFormat("yy-MM-dd");
            return dayformat.format(timeInMillis);
        }
        else if (Dduration > 0){
            return Dduration+"天前";
        }
        else{
            long Hduration = duration /(1000*3600);   //计算小时的间隔
            if (Hduration > 0){
                return Hduration+"小时前";
            }
            else{
                long Mduration = duration /(1000*60);   //计算分钟的间隔
                if (Mduration > 0){
                    return Mduration+"分钟前";
                }
                else{
                    return "刚刚";
                }
